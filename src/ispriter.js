...
function combineSpriteTasks(spriteTaskArray){
    ...
    spriteTaskArray.forEach(function(spriteTask){
        ...
        styleSheetFileNameArray.push(spriteTask.cssFileName);
    });

    combinedSpriteTask = {
        cssFileName: combinedFileName,
        cssFileNameArray: styleSheetFileNameArray,
...
function exportCssFile(spriteTask){
    var cssContentList = [],
        ...
        fileNameArray = spriteTask.cssFileNameArray,
        cssContent = '',
    ...
    if(spriteConfig.output.splitCss){
        cssContentList.forEach(function( curCssContent, index ){
            ...
            var absoluteFileName = path.join(spriteConfig.output.cssDist, curFileName);
            absoluteFileName = path.normalize(absoluteFileName);
            absoluteFileName = path.join(spriteConfig.workspace, absoluteFileName);
            absoluteFileName = path.resolve(absoluteFileName);
            if (compressOptions) { // 压缩
                if (!us.isObject(compressOptions)) {
                    compressOptions = null;
                }
                tempCssCont = new CleanCSS(compressOptions).minify(tempCssCont);
            }


            nf.writeFileSync(absoluteFileName, tempCssCont, true);
            info('>>Output css:', curFileName);
        })
